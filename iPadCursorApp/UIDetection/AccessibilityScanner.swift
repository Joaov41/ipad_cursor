...
/// Wrapper around the Accessibility API that fetches interactive UI elements.
final class AccessibilityScanner {
    private let systemWideElement = AXUIElementCreateSystemWide()

    func element(at point: CGPoint) -> AccessibilityElement? {
        var rawElement: AXUIElement?
        let status = AXUIElementCopyElementAtPosition(systemWideElement, Float(point.x), Float(point.y), &rawElement)
        guard status == .success, let rawElement else { return nil }
        return makeElement(from: rawElement)
    }

    func elements(around point: CGPoint, radius: CGFloat) -> [AccessibilityElement] {
        // Accessibility API does not provide direct spatial queries, so we sample a grid
        // around the point to find nearby focusable elements.
        ...
        stride(from: -radius, through: radius, by: step).forEach { dx in
            stride(from: -radius, through: radius, by: step).forEach { dy in
                ...
                if visited.insert(rawElement).inserted, let element = makeElement(from: rawElement) {
    ...
    private func makeElement(from rawElement: AXUIElement) -> AccessibilityElement? {
        ...
        guard AccessibilityElement.isInteractive(role: role, subrole: subrole, supportsPress: supportsPress, inCollection: inCollection) else {
            return nil
        }
